-- ============================================================================
-- FLEXFIT SYSTEM FULL SEED DATA SCRIPT
-- Roles: Admin, GymPartner, Staff, Member (Total 4 Roles)
-- Password Default for All Seeded Users: 123456aA@
-- Salt & Hash Format: Pbkdf2 HMACSHA256 (SaltSize: 16, HashSize: 32, Iterations: 10000)
-- Salt: +bXkP319nN/tO4R62J87FA==
-- Hash: g38+Fk5/8hP8mC3O80f33d7xM6wO8w6s47587g0+98g=
-- PasswordHash string: +bXkP319nN/tO4R62J87FA==.g38+Fk5/8hP8mC3O80f33d7xM6wO8w6s47587g0+98g=
-- ============================================================================
...
BEGIN TRY
    ...
    -- ------------------------------------------------------------------------
    -- 0. CLEANUP EXISTING DATA (IN REVERSE FOREIGN KEY ORDER)
    -- ------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------
    DECLARE @DefaultPasswordHash NVARCHAR(MAX) = N'+bXkP319nN/tO4R62J87FA==.g38+Fk5/8hP8mC3O80f33d7xM6wO8w6s47587g0+98g=';
    ...
    -- Fix description typo for Pilates
    UPDATE Categories SET CategoryName = N'Pilates' WHERE CategoryId = @Cat_Pilates;

    -- ------------------------------------------------------------------------
    -- 4. SEED GYMS, BRANCHES & STAFF ASSIGNMENTS
    -- ------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------
    -- 6. SEED CREDIT PACKAGES & USER CREDITS
    -- ------------------------------------------------------------------------
    ...
    INSERT INTO GymBookings (BookingId, UserId, SessionId, BookingCode, CreditUsed, CheckInStatus, Status, BookedAt, CheckedInBy, CheckInTime)
    VALUES
    (@Booking_Gym1, @UserId_Member1, @Session_Q1_Morning, N'GBK-998231', 15, 'CheckedIn', 'Completed', DATEADD(DAY, -1, GETUTCDATE()), @UserId_StaffQ1, DATEADD(HOUR, -24, GETUTCDATE()));

    INSERT INTO ClassBookings (BookingId, UserId, ClassId, BookingCode, CreditUsed, CheckInStatus, Status, BookedAt)
    VALUES
    (@Booking_Class1, @UserId_Member2, @Class_YogaQ1, N'CBK-114820', 18, 'NotCheckedIn', 'Booked', GETUTCDATE());
    ...
    -- ------------------------------------------------------------------------
    -- 8. SEED ENGAGEMENT & RECOMMENDATIONS (REVIEWS, FAVORITES, HISTORIES)
    -- ------------------------------------------------------------------------
    ...
    COMMIT TRANSACTION;
    PRINT N'=== SEED DATA FULLY EXECUTED SUCCESSFULLY FOR FLEXFIT PROJECT ===';

END TRY
BEGIN CATCH
    ROLLBACK TRANSACTION;
    PRINT N'ERROR SEEDING DATA: ' + ERROR_MESSAGE();
END CATCH;
GO
