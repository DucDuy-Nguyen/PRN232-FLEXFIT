...
public static class PromptBuilder
{
    public static string BuildPrompt(AIUserContextDto context, string userQuestion)
    {
        ...
        if (context.Role.Contains("Member", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (context.FavoriteGyms.Any())
            {
                foreach (var g in context.FavoriteGyms)
                {
                    sb.AppendLine($"- {g}");
                }
            }
            ...
            {
                sb.AppendLine("- Chưa có phòng tập yêu thích.");
            }

            sb.AppendLine("Favorite Classes:");
            if (context.FavoriteClasses.Any())
            {
                foreach (var c in context.FavoriteClasses)
                {
                    sb.AppendLine($"- {c}");
                }
            }
            ...
            {
                sb.AppendLine("- Chưa có lớp học yêu thích.");
            }

            sb.AppendLine("Recent Bookings (Gym and Classes):");
            if (context.RecentBookings.Any())
            {
                foreach (var b in context.RecentBookings)
                {
                    sb.AppendLine($"- {b}");
                }
            }
            ...
            {
                sb.AppendLine("- Chưa có lịch đặt chỗ nào.");
            }

            sb.AppendLine("Recent Workout History:");
            if (context.WorkoutHistorySummary.Any())
            {
                foreach (var w in context.WorkoutHistorySummary)
                {
                    sb.AppendLine($"- {w}");
                }
            }
            ...
            {
                sb.AppendLine("- Chưa có lịch sử tập luyện.");
            }
        }
        ...
        if (context.Role.Contains("Partner", StringComparison.OrdinalIgnoreCase) || context.Role.Contains("GymPartner", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (context.OwnedGyms.Any())
            {
                foreach (var gym in context.OwnedGyms)
                {
                    sb.AppendLine($"- {gym}");
                }
            }
            ...
            {
                sb.AppendLine("- Bạn chưa đăng ký sở hữu phòng tập nào.");
            }
        ...
        if (context.Role.Contains("Staff", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (context.ManagedBranches.Any())
            {
                foreach (var mb in context.ManagedBranches)
                {
                    sb.AppendLine($"- {mb}");
                }
            }
            ...
            {
                sb.AppendLine("- Bạn chưa được phân công quản lý chi nhánh nào.");
            }
