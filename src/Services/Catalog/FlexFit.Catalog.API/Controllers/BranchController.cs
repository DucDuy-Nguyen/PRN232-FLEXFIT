...
public class BranchController : ControllerBase
{
    ...
    [HttpPost("assign-staff-by-email")]
    [Authorize(Roles = "GymPartner")]
    public async Task<IActionResult> AssignStaffToBranchByEmail([FromBody] AssignStaffByEmailDto dto)
    {
        try
        {
    ...
    public async Task<IActionResult> RemoveStaffFromBranch([FromQuery] Guid staffId, [FromQuery] Guid branchId)
    {
        try
        {
            ...
            return Ok(new { message = "Đã gỡ nhân viên ra khỏi chi nhánh thành công!" });
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
    ...
    public async Task<IActionResult> UpdateBranchStaff([FromBody] UpdateBranchStaffDto dto)
    {
        try
        {
            ...
            return Ok(new { message = "Đã chuyển giao quyền quản lý chi nhánh thành công!" });
        }
        catch (KeyNotFoundException ex)
        {
            return NotFound(new { message = ex.Message });
