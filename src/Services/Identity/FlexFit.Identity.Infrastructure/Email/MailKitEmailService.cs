...
namespace FlexFit.Identity.Infrastructure.Email;

public sealed class MailKitEmailService : IEmailService
{
    private readonly Security.EmailOptions _options;
    private readonly ILogger<MailKitEmailService> _logger;

    public MailKitEmailService(IOptions<Security.EmailOptions> options, ILogger<MailKitEmailService> logger)
    {
        _options = options?.Value ?? throw new ArgumentNullException(nameof(options));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));

        // Validate SMTP Options at startup
    ...
    public async Task SendEmailAsync(string toEmail, string subject, string body, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Avoid connecting/sending real emails during testing via checking environment or options if needed, 
            // but standard code should execute SMTP calls. We will handle testing using a mock/override or unit tests.
        ...
        {
            // Never log the email password/credentials or OTP contents in the exception logs
    ...
    {
        // Choose subject and title based on purpose
        ...
        // Log transaction initiation without detailing sensitive items like the OTP itself
