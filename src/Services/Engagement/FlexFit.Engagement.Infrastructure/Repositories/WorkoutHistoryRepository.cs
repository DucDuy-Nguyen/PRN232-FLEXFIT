...
public class WorkoutHistoryRepository : IWorkoutHistoryRepository
{
    ...
    public WorkoutHistoryRepository(EngagementDbContext context) { _context = context; }

    public async Task AddAsync(UserWorkoutHistory history) => await _context.UserWorkoutHistories.AddAsync(history);

    public async Task<UserWorkoutHistory?> GetByIdAsync(Guid historyId)
        => await _context.UserWorkoutHistories.FirstOrDefaultAsync(h => h.WorkoutHistoryId == historyId);
    ...
    public async Task<bool> ExistsByClassBookingIdAsync(Guid classBookingId)
        => await _context.UserWorkoutHistories.AnyAsync(h => h.ClassBookingId == classBookingId);

    public async Task<bool> ExistsByGymBookingIdAsync(Guid gymBookingId)
        => await _context.UserWorkoutHistories.AnyAsync(h => h.GymBookingId == gymBookingId);

    public async Task UpdateAsync(UserWorkoutHistory history) { _context.UserWorkoutHistories.Update(history); await Task.CompletedTask; }
