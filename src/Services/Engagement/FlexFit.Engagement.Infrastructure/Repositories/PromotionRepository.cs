...
public class PromotionRepository : IPromotionRepository
{
    ...
    public PromotionRepository(EngagementDbContext context) { _context = context; }
    ...
    public async Task<Promotion?> GetByIdAsync(Guid id)
        => await _context.Promotions.FirstOrDefaultAsync(p => p.PromotionId == id);

    public async Task AddAsync(Promotion promotion) => await _context.Promotions.AddAsync(promotion);

    public async Task DeleteAsync(Promotion promotion) { _context.Promotions.Remove(promotion); await Task.CompletedTask; }
